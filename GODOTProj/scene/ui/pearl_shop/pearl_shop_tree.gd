...
@onready var tree = $MarginContainer/VBoxContainer/HBoxContainer/VBoxContainer/TreeScroll/HBoxContainer
	...
	for box in tree.get_children():
		for node in box.get_children():
			if node.has_signal("buy_requested"):
				node.buy_requested.connect(_on_card_buy_requested)
...
func refresh_shop() -> void:
	...
	for box in tree.get_children():
		for node in box.get_children():
			if node.has_method("update_card"):
				var level = 0
			
				match node.upgrade_id:
				...
				node.update_card(level, current_pearls)
